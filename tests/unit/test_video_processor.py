...
# ═══════════════════════════════════════════════════════════════════════
# Multi-segment portrait conversion (RFN-31)
# ═══════════════════════════════════════════════════════════════════════
...
@patch("pipeline.video_processor.speaker_layout.analyze")
@patch("pipeline.video_processor.subprocess.run")
def test_convert_to_portrait_single_segment_no_concat(
    mock_run: MagicMock,
    mock_analyze: MagicMock,
    mock_probe: MagicMock,
    mock_cuda: MagicMock,
    ...
    """Single segment → renders directly (no concat."""
    from pipeline.speaker_layout import LayoutSegment, LayoutMode
    ...
    # Only one FFmpeg call — no concat
...
@patch("pipeline.video_processor.speaker_layout.analyze")
@patch("pipeline.video_processor.subprocess.run")
def test_convert_to_portrait_multi_segment_uses_concat(
    mock_run: MagicMock,
    mock_analyze: MagicMock,
    mock_probe: MagicMock,
    mock_cuda: MagicMock,
    ...
    """Two segments (SINGLE→SPLIT) → renders 2 clips + 1 concat = 3 ffmpeg calls."""
    from pipeline.speaker_layout import LayoutSegment, LayoutMode
    ...
    mock_run.return_value = _completed_proc(returncode=0)
    mock_analyze.return_value = [
        LayoutSegment(0.0, 5.0, LayoutMode.SINGLE, [(120, 840, 420)]),
        LayoutSegment(5.0, 10.0, LayoutMode.SPLIT, [(0, 720, 100), (120, 840, 840)]),
    ]
    ...
    # 2 segment renders + 1 concat = 3 ffmpeg calls
    assert mock_run.call_count == 3, f"Expected 3 ffmpeg calls, got {mock_run.call_count}"

    # Verify the final call uses concat demuxer
    final_call_args = mock_run.call_args_list[-1][0][0]
    assert "-f" in final_call_args
    assert "concat" in final_call_args or "--concat" in final_call_args
    # Check segment timing was passed to renders
    first_call_args = mock_run.call_args_list[0][0][0]
    assert "-ss" in first_call_args, "first segment should use -ss"
    assert "0.0" in first_call_args, "first segment should start at 0.0"
...
@patch("pipeline.video_processor.speaker_layout.analyze")
@patch("pipeline.video_processor.subprocess.run")
def test_convert_to_portrait_segment_timing_passed_correctly(
    mock_run: MagicMock,
    mock_analyze: MagicMock,
    mock_probe: MagicMock,
    mock_cuda: MagicMock,
    ...
    """Each segment gets -ss with start_sec and -t with correct duration."""
    from pipeline.speaker_layout import LayoutSegment, LayoutMode
    ...
    mock_run.return_value = _completed_proc(returncode=0)
    mock_analyze.return_value = [
        LayoutSegment(2.5, 7.0, LayoutMode.SINGLE, [(120, 840, 420)]),
    ]
